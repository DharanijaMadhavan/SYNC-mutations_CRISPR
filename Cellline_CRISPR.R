...
ceres.dist <-  ggplot(gene_ceres, aes(x = Gene)) + geom_histogram(colour= "black", fill="white") + 
  ...
  geom_vline(aes(xintercept = Gene), gene_mut_ceres, col=colour)+
   annotate("text", x=gene_mut_ceres$Gene, y=70, label=labels, angle=90, vjust=-0.5, size=2) #only if legible
...
Cell.lines_crispr <- Cell.lines[na.omit(match(row.names(depmap_ceres),Cell.lines$DepMap_ID)),
                                c("CCLE_Name","Protein_Change")] 
                        
  
Cell.lines_crispr <- paste(Cell.lines_crispr$CCLE_Name, Cell.lines_crispr$Protein_Change, sep=":")
...
plot_combine[1:length("plot_volcano")] <- 
          lapply(1:length("plot_volcano"), function(i) arrangeGrob(plot_volcano[[i]],
                                                                   top = title[i]))


##############################
#Table of synthetically lethal genes
##############################

sl <- sapply(mut_ceres_pval, function(x){subset(x, x$FDR < 0.1)$Gene}) 
names(sl) <- Cell.lines_crispr
sl <- c(sl, "Common" = list(as.factor(Reduce(intersect, sl))))
str(sl)          
...
Summary <- rbind(c(Gene, paste(mutations, collapse=",") , nrow(mut.depmap2), 
                            length(Cell.lines_crispr)), Summary)
colnames(Summary) <- c("Gene", "Sync Mutations","#Cell lines with SYNC mutations",
